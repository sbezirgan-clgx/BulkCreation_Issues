...
for index, row in df.iterrows():
    ...
    QA_Regular_Format1_3072 = {
        ...
        'customfield_24512': {'value': LookUp.thirtyseventytwo_Values[lookup_key]},
        'customfield_29815': ' ' if math.isnan(row['Record ID']) else str(row['Record ID']),  # RecordID comes up as NaN, should be blank----------
        ...
        'customfield_24526': format_the_date(row['Recording date']),  # Recording Date,watch out for the format---------
        ...
        'customfield_24529': format_the_date(row['Detected Date']),  # Should be transmission date-------
        'customfield_24528': row['Sample data '],  # Sample Data----------
        'customfield_24506': {'value': row['Critical/Non-Critical']},  # Critical----------
        'customfield_24515': format_the_date(row['BatchDate']), # Batch Date but pay attention to the format in excel--------
